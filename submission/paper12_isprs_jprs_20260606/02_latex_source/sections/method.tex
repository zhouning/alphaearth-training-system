...
We evaluate seven adaptation strategies. Throughout the paper, \emph{standard LoRA} denotes a module-wise insertion rule that wraps visible linear submodules in the implementation. \emph{Split-QKV LoRA} denotes the diagnostic variant in which the packed attention projection is explicitly decomposed before low-rank updates are inserted.
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{LoRA (r=8)}~\cite{hu2022lora}: standard low-rank adaptation using the module-wise insertion rule above. This variant is included because it is the most likely implementation pattern in applied PyTorch workflows and is therefore a useful test for silent insertion failure.
...
The experiments are designed to test PEFT behavior for Prithvi-100M rather than to establish a universal ranking for all geospatial foundation models. The study therefore makes three bounded claims. First, the fused-QKV LoRA failure is an implementation risk for Prithvi-like PyTorch attention modules, not a proof that LoRA is ineffective on every remote sensing backbone. Second, the Houlsby advantage is established under the parameter budgets and datasets reported here; parameter-matched LoRA and adapter-width sweeps remain necessary before attributing the entire gain to adapter architecture rather than capacity. Third, the Linhe labels are derived from an external land-cover product and should be interpreted as production-style supervisory labels rather than manually annotated ground truth.

\subsection{Capacity-Audit Extension Prepared for Revision}
To address the parameter-budget confound directly, we prepared a parameter-capacity audit that can be run without changing the training framework. The audit uses EuroSAT s2\_full, the same Prithvi-100M checkpoint, the same seeds $\{42,123,456\}$, and the same OA/macro-F1 metrics. It compares split-QKV LoRA ranks $r\in\{4,8,16,32,64\}$ against Houlsby bottleneck widths $d\in\{8,16,32,64\}$, with linear probing as the frozen-backbone floor. The Colab notebook and config are released as \texttt{colab/paper12\_peft\_capacity\_sweep\_colab.ipynb} and \texttt{geoadapter/bench/configs/eurosat\_peft\_capacity\_sweep.yaml}. We treat this audit as the decisive next step for separating adapter placement from parameter count; until the sweep is run, capacity-boundary language in this paper is interpreted as a supported hypothesis rather than a general rule.
