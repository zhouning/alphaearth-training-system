...
\caption{EuroSAT s2\_full overall accuracy versus trainable parameter count (log scale). Houlsby adapters reach 94.5\% of the full fine-tuning ceiling using 1.4\% of the parameters; standard LoRA remains at the linear-probe baseline in this implementation.}
...
The benchmark supports three practical recommendations within the tested Prithvi-100M setting. First, Houlsby adapters are the safest default among the evaluated methods for adapting Prithvi-100M to heterogeneous inputs. Second, LoRA should not be assumed effective on fused-QKV backbones without careful architectural inspection and a corrected split-QKV control. Third, when spectral richness is available, preserving that information has larger returns than switching between weak PEFT methods.
