...
- Verify the completed second-backbone EuroSAT artifacts from `colab/paper12_second_backbone_eurosat_colab.ipynb`: `paper12_results/second_backbone_eurosat.json` and `paper12_results/second_backbone_eurosat_summary.json`.
- Confirm both second-backbone files are mirrored into `06_supplementary_material/paper12_results/` before final upload.
...
- Regenerate `paper12_results/review_audit_summary.json` with `python -m geoadapter.bench.paper12_audit` after any result-file change, and mirror it into `06_supplementary_material/paper12_results/`. Confirm schema version 2 includes model-scope, second-backbone, label-source, and decoder-capacity checks.
