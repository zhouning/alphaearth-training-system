...
\paragraph{Small-PEFT methods cluster at the majority-class baseline under domain shift.} The four small-parameter methods --- linear probing (5.4k params), BitFit (128k), LoRA r=8 (153k), and GeoAdapter (5.5k) --- cluster within $\pm 0.001$ mIoU of one another in the U$\rightarrow$R direction at the 0.085--0.086 range, which lies within $1 \times 10^{-3}$ of the all-class-0 baseline (0.0858) computed by predicting only the dominant background class against the LoveDA validation class distribution. The same pattern holds R$\rightarrow$U at the 0.095--0.102 cluster, also within $10^{-2}$ of its respective majority-class floor. We instrument the streaming evaluator to report per-class IoU and the predicted-pixel histogram alongside aggregated mIoU (Appendix~\ref{app:loveda-perclass}); the recovered per-class signature distinguishes majority-class collapse from weak-signal under-reporting. To rule out the alternative reading that small PEFT was undertrained at the protocol-default schedule, we run a single-seed sweep enlarging LoRA's learning rate by 5$\times$ and the training budget to 60 epochs (Appendix~\ref{app:loveda-perclass}).

\paragraph{A capacity threshold separates working PEFT from collapsed PEFT under domain shift.} Houlsby's 1.2M trainable parameters are the only configuration in our sweep that escapes the majority-class floor on LoveDA cross-domain. BitFit at 128k parameters and LoRA r=8 at 153k --- both 7$\times$ to 9$\times$ larger than the linear probe --- behave statistically indistinguishably from linear probing. This is qualitatively different from the within-domain Linhe results (where BitFit was $+0.020$ and GeoAdapter $+0.087$ above linear probing); the cross-domain protocol appears to impose a per-method capacity threshold below which adapters cannot recover discriminative content from a frozen Prithvi backbone whose pretraining distribution does not include the LoveDA scene statistics. This is, to our knowledge, the first systematic per-method capacity-threshold report for PEFT on a public remote-sensing cross-domain benchmark; it is also actionable: a practitioner deploying a frozen geospatial foundation model on a target domain that drifts substantially from pretraining should budget on the order of $10^6$ trainable adapter parameters, not the $10^5$ commonly reported as ``parameter-efficient'' on within-domain transfer.
...
\begin{enumerate}[leftmargin=1.5em,itemsep=0.2em]
    ...
    \item \emph{Domain shift imposes a per-method capacity threshold.} Section~\ref{sec:loveda} shows that under LoveDA cross-domain, every PEFT method below $\approx$$10^6$ trainable parameters collapses to the majority-class baseline. This is not visible on within-domain benchmarks, where small-parameter methods like BitFit and GeoAdapter recover meaningful gains. Practitioners should budget adapter capacity by target-domain drift, not only by within-domain accuracy economy.
