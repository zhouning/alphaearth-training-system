...
\begin{enumerate}
    ...
    \item A 30-run cross-domain replication on LoveDA (5 methods $\times$ 2 directions $\times$ 3 seeds) reporting a per-method capacity threshold near $10^6$ trainable parameters. Houlsby reaches $+0.117$ mIoU over linear probing under R$\rightarrow$U transfer; every PEFT method below the threshold (linear probing, BitFit, LoRA r=8/16, GeoAdapter) plateaus within $10^{-3}$ mIoU of the all-background baseline despite a 7--9$\times$ trainable-parameter advantage over linear probing. A per-class diagnostic at 5$\times$ learning rate and 60 epochs confirms that sub-threshold adapters recover at most a binary background-versus-foreground distinction rather than collapsing or being undertrained, giving practitioners an actionable adapter-capacity rule for cross-domain deployment that within-domain benchmarks do not surface.
\end{enumerate}
